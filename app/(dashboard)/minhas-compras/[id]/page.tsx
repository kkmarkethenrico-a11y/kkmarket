...
export default async function MinhaCompraDetalhe({ params }: PageProps) {
  ...
  const { data: order } = await admin
    ...
    .select(`
      ...
      mp_payment_id, escrow_release_at, buyer_confirmed_at, completed_at,
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className="container mx-auto max-w-4xl px-4 py-8 space-y-6">
        ...
        {/* Voltar */}
        <Link href="/minhas-compras" className="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-300 transition-colors">
          ← Minhas compras
        </Link>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-white">Pedido #{order.id.slice(0, 8).toUpperCase()}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {seller && (
                    <div className="flex items-center gap-2 mt-3">
                      {seller.avatar_url ? (
                        <Image src={seller.avatar_url} alt={seller.username} width={24} height={24} className="rounded-full object-cover" />
                      ) : (
                      ...
                      <span className="text-sm text-zinc-400">
                        Vendido por{' '}
                        <Link href={`/perfil/${seller.username}`} className="text-violet-400 hover:underline">
                          @{seller.username}
                        </Link>
                      </span>
                    </div>
            ...
            {order.status === 'cancelled' && order.cancellation_reason && (
              <div className="rounded-2xl border border-zinc-700 bg-zinc-900/40 p-5">
                <p className="text-sm font-semibold text-zinc-300">Pedido cancelado</p>
                <p className="text-xs text-zinc-500 mt-1">{order.cancellation_reason}</p>
              </div>
            )}

            {/* Ações do comprador */}
            <BuyerActions orderId={order.id} status={order.status} />
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-3">
              <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Pagamento</h2>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-zinc-500">Total pago</span>
                  <span className="font-bold text-white">{fmtBRL(Number(order.amount))}</span>
                </div>
                {order.payment_method && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Forma</span>
                    <span className="text-zinc-300">{PAYMENT_LABELS[order.payment_method] ?? order.payment_method}</span>
                  </div>
                )}
                {order.mp_payment_id && (
                  <div className="flex justify-between gap-2">
                    <span className="text-zinc-500 shrink-0">ID MP</span>
                    <span className="text-zinc-500 font-mono text-xs truncate">{order.mp_payment_id}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-3">
              ...
              <div className="space-y-2 text-xs text-zinc-500">
                ...
                {order.buyer_confirmed_at && (
                  <div>
                    <span className="block text-zinc-600">Confirmado em</span>
                ...
                {order.escrow_release_at && !order.completed_at && (
                  <div>
                    <span className="block text-zinc-600">Liberação escrow</span>
                    <span className="text-zinc-300">{fmtDateTime(order.escrow_release_at)}</span>
                  </div>
                )}
              </div>
            </div>
