...
function planWeight(plan: string) { return PLAN_ORDER[plan] ?? 0 }
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default async function TopCategoryPage({ params, searchParams }: Props) {
  const { slug } = await params
  ...
  const allCatIds = [topCat.id, ...subcategories.map((c) => c.id)]
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          ...
          <main className="flex min-w-0 flex-1 flex-col gap-6">
            ...
            {announcements.length > 0 ? (
              ...
              <EmptyState catName={(topCat as Category).name} subcategories={subcategories} slug={slug} />
            )}
